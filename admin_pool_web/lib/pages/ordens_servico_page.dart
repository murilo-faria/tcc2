...
class _OrdensServicoPageNovaState extends State<_OrdensServicoPageNova> {
  ...
  Future<List<dynamic>> _buscar(String rota) async {
    ...
    if (resposta.statusCode != 200) throw Exception('Não foi possível carregar os dados.');
      ...
      builder: (context) => StatefulBuilder(
        builder: (_, setLocal) => AlertDialog(
              ...
              child: Column(
                ...
                children: [
                  DropdownButtonFormField<int>(
                    ...
                    items: clientes.map<DropdownMenuItem<int>>((cliente) => DropdownMenuItem(value: cliente['id'], child: Text(cliente['nome']))).toList(),
                    onChanged: (id) async {
                      final novasPiscinas = await _buscar('/api/piscinas/cliente/$id');
                      setLocal(() {
                        clienteId = id!;
                        piscinas = novasPiscinas;
                        piscinaId = novasPiscinas.length == 1 ? novasPiscinas.first['id'] as int : null;
                      });
                    },
                  ),
                  DropdownButtonFormField<int>(
                    ...
                    items: piscinas.map<DropdownMenuItem<int>>((piscina) => DropdownMenuItem(value: piscina['id'], child: Text(piscina['nome']))).toList(),
                    onChanged: (id) => setLocal(() => piscinaId = id),
                  ),
                  TextField(controller: descricao, maxLines: 4, decoration: const InputDecoration(labelText: 'Descrição do serviço *')),
                  TextField(
          ...
          actions: [
            ...
            FilledButton(onPressed: piscinaId == null ? null : () => Navigator.pop(context, true), child: const Text('Criar OS')),
          ],
        ),
      ),
    );
    if (salvar != true || descricao.text.trim().isEmpty || piscinaId == null) return;
    final resposta = await apiService.post('/api/ordens-servico', body: {
      'clienteId': clienteId,
      'piscinaId': piscinaId,
      'descricao': descricao.text.trim(),
      'dataServico': DateTime.now().toIso8601String().substring(0, 10),
      'valorAdicional': double.tryParse(valorCobrado.text.replaceAll(',', '.')) ?? 0,
    });
    if (resposta.statusCode >= 200 && resposta.statusCode < 300) _recarregar();
  }

  Future<void> _concluir(Map<String, dynamic> ordem) async {
    final custo = TextEditingController(text: (ordem['valorCusto'] ?? 0).toString());
    final cobrado = TextEditingController(text: (ordem['valorCobrado'] ?? ordem['valorAdicional'] ?? 0).toString());
    ...
    final confirmar = await showDialog<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (_, setLocal) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  TextField(controller: custo, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Valor pago/custo', prefixText: 'R\$ ')),
                  TextField(controller: cobrado, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Valor cobrado do cliente', prefixText: 'R\$ ')),
                  const SizedBox(height: 12),
                  RadioListTile<String>(
                    value: 'EMPRESA', groupValue: pagador,
                    title: const Text('Pago pela empresa'),
                    subtitle: const Text('Entra na cobrança do cliente, sem reembolso.'),
                    onChanged: (v) => setLocal(() => pagador = v!),
                  ),
                  RadioListTile<String>(
                    value: 'FUNCIONARIO', groupValue: pagador,
                    title: const Text('Pago pelo funcionário'),
                    subtitle: Text(temCriador ? 'Gera cobrança e reembolso para quem criou a OS.' : 'Indisponível: esta OS não possui funcionário criador.'),
                    onChanged: temCriador ? (v) => setLocal(() => pagador = v!) : null,
                  ),
                  RadioListTile<String>(
                    value: 'CLIENTE', groupValue: pagador,
                    title: const Text('Pago pelo cliente'),
                    subtitle: const Text('Já está quitado: não entra em cobrança nem gera reembolso.'),
                    onChanged: (v) => setLocal(() => pagador = v!),
                  ),
                  const Divider(),
                  const Text('Ao confirmar, os lançamentos financeiros serão criados conforme a opção escolhida.'),
                ],
              ),
            ),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancelar')),
            FilledButton.icon(onPressed: () => Navigator.pop(context, true), icon: const Icon(Icons.check_circle_outline), label: const Text('Confirmar conclusão')),
          ],
        ),
      ),
    );
    if (confirmar != true) return;
    final resposta = await apiService.put('/api/ordens-servico/${ordem['id']}/concluir', body: {
    ...
    if (resposta.statusCode >= 200 && resposta.statusCode < 300) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Não foi possível concluir a OS (${resposta.statusCode}).')));
    }
  }
    ...
    final confirmar = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Cancelar OS')),
        ],
      ),
    );
    if (confirmar != true) return;
    final resposta = await apiService.put('/api/ordens-servico/${ordem['id']}/cancelar');
    if (resposta.statusCode >= 200 && resposta.statusCode < 300) _recarregar();
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              Text(ordem['descricao'] ?? '', style: const TextStyle(fontSize: 16)),
              const Divider(),
              Text('Cliente: ${(ordem['cliente'] ?? {})['nome'] ?? ''}'),
              Text('Piscina: ${(ordem['piscina'] ?? {})['nome'] ?? ''}'),
              Text('Data: ${ordem['dataServico'] ?? '-'}'),
              Text('Custo: ${formatarMoeda((ordem['valorCusto'] as num?) ?? 0)}'),
              Text('Cobrado: ${formatarMoeda((ordem['valorCobrado'] as num?) ?? 0)}'),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            decoration: const InputDecoration(prefixIcon: Icon(Icons.search), labelText: 'Pesquisar cliente ou serviço', border: OutlineInputBorder()),
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, indice) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text('${ordem['descricao']}\n${ordem['dataServico']} • ${ordem['status']}'),
                        isThreeLine: true,
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            if ((ordem['valorCobrado'] as num?) != null) Text(formatarMoeda(ordem['valorCobrado'] as num)),
                            IconButton(tooltip: 'Ver detalhes', onPressed: () => _detalhar(ordem), icon: const Icon(Icons.visibility_outlined)),
                            if (widget.gestor && aberta) IconButton(tooltip: 'Concluir OS', onPressed: () => _concluir(ordem), icon: const Icon(Icons.check_circle_outline, color: Colors.green)),
                            if (aberta) IconButton(tooltip: 'Cancelar OS', onPressed: () => _cancelar(ordem), icon: const Icon(Icons.cancel_outlined, color: Colors.red)),
                          ],
                        ),
                      );
                    },
