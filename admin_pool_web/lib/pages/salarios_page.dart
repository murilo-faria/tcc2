...
class _SalariosPageNovaState extends State<_SalariosPageNova> {
  ...
  String get _referencia =>
      '${_mesSelecionado.year}-${_mesSelecionado.month.toString().padLeft(2, '0')}';
  ...
  bool _mesIgual(DateTime primeiro, DateTime segundo) =>
      primeiro.year == segundo.year && primeiro.month == segundo.month;
  ...
  Future<List<dynamic>> _carregar() async {
    final resposta = await apiService.get(
      '/api/salarios?referencia=$_referencia',
    );
  ...
  Future<void> _abrirVales() async {
    await showDialog<void>(
      context: context,
      builder: (_) => _DialogoVales(
        aoAtualizar: () => setState(() => _salarios = _carregar()),
      ),
    );
  }
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  CircleAvatar(
                    backgroundColor: _cor(funcionario).withValues(alpha: .15),
                    foregroundColor: _cor(funcionario),
              ...
              const SizedBox(height: 6),
              Text(
                '${resumo['piscinas']} piscina(s) • ${resumo['clientes']} cliente(s)',
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            widget.gestor ? 'Salários' : 'Meu salário',
            style: Theme.of(
              context,
          ...
          Text(
            'Referência: ${_nomeMes(_mesSelecionado)}',
            style: Theme.of(
              context,
...
class _DialogoValesState extends State<_DialogoVales> {
  ...
  Future<List<dynamic>> carregar() async {
    ...
    if (resposta.statusCode != 200)
      throw Exception('Não foi possível carregar os vales.');
    return jsonDecode(resposta.body) as List<dynamic>;
  }
            ...
            children: [
              DropdownButtonFormField<int>(
                ...
                items: lista
                    ...
                    .toList(),
                onChanged: vale == null
                    ? (value) => setDialogState(() => funcionario = value!)
                    : null,
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(
                    value: 'DINHEIRO_CLIENTE',
                    child: Text('Dinheiro recebido de cliente'),
                  ),
                ],
                  ...
                  leading: CircleAvatar(
                    backgroundColor:
                        [
                          ...
                          Colors.orange,
                          Colors.deepPurple,
                        ][(funcionario['id'] as int? ?? 0) % 4].withValues(
                          alpha: .15,
                        ),
                    child: const Icon(Icons.person_outline),
                  ),
                  ...
                  trailing: celular
                      ? PopupMenuButton<String>(
                          ...
                          itemBuilder: (_) => const [
                            PopupMenuItem(
                              value: 'editar',
                              child: Text('Editar vale'),
                            ),
                            PopupMenuItem(
                              value: 'excluir',
                              child: Text('Excluir vale'),
                            ),
                          ],
                        )
                      : Row(
                          ...
                          children: [
                            Text(valor),
                            IconButton(
                              onPressed: () => formulario(vale),
                              icon: const Icon(Icons.edit_outlined),
                            ),
                            IconButton(
                              onPressed: () => excluir(vale),
                              icon: const Icon(
...
class _DialogoReembolsosState extends State<_DialogoReembolsos> {
  ...
  Future<List<dynamic>> _carregar() async {
    final resposta = await apiService.get(
      '/api/salarios/${widget.funcionario['id']}/reembolsos?referencia=${widget.referencia}',
    );
    if (resposta.statusCode != 200)
      throw Exception('Não foi possível carregar os reembolsos.');
    return jsonDecode(resposta.body) as List<dynamic>;
  }

  Future<List<dynamic>> _carregarVales() async {
    final resposta = await apiService.get(
      '/api/salarios/vales?funcionarioId=${widget.funcionario['id']}&referencia=${widget.referencia}',
    );
    if (resposta.statusCode != 200)
      throw Exception('Não foi possível carregar os vales.');
    return jsonDecode(resposta.body) as List<dynamic>;
  ...
  Future<void> _pagar(Map<String, dynamic> reembolso) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Confirmar o pagamento de ${formatarMoeda((reembolso['valor'] as num?) ?? 0)} ao colaborador?',
        ),
        actions: [
          TextButton(
    ...
    final resposta = await apiService.put(
      '/api/salarios/reembolsos/${reembolso['id']}/pagar',
    );
