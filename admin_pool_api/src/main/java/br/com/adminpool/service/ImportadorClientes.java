...
public class ImportadorClientes implements CommandLineRunner {
    ...
    private void adicionarProduto(String nome, String compra, String venda) { if (!produtos.existsByNomeIgnoreCase(nome)) { Produto p = new Produto(); p.setNome(nome); p.setPrecoCompra(new BigDecimal(compra)); p.setPrecoVenda(new BigDecimal(venda)); produtos.save(p); } }
}
